...
router.get('/theme', async (req, res) => {
    try {
        ...
        console.error('[THEME] Error getting theme:', error);
        res.json({ defaultTheme: 'dark' });
    }
});

router.put('/theme', [auth, requireSettingsRoles, verifyCsrfToken], async (req, res) => {
    try {
        const { defaultTheme } = req.body;
        
        const validThemes = ['dark', 'blue', 'purple', 'green', 'orange', 'teal', 'cyberpunk', 'slate-professional', 'emerald-pro'];
        ...
        const settings = await Settings.findOneAndUpdate(
            ...
            { 
                $set: { 
                    'dashboardSettings.defaultTheme': defaultTheme 
                }
            },
            { upsert: true, new: true }
        );

        res.json({ defaultTheme: settings.dashboardSettings.defaultTheme });
    } catch (error) {
        res.status(500).json({ error: 'Failed to update default theme' });
    }
});

router.use('/channel-stats', channelStatsRoutes);

router.get('/blacklist-words', [auth, requireSettingsRoles], async (req, res) => {
  try {
    const settings = await Settings.findOne({ guildId: req.user.guildId });
    ...
    res.json(settings.blacklistWords);
  } catch (error) {
    console.error('[ERROR] Failed to fetch blacklist words settings:', error);
    res.status(500).json({ error: 'Failed to fetch blacklist words settings' });
  }
});

router.post('/blacklist-words', [auth, requireSettingsRoles, verifyCsrfToken], async (req, res) => {
  try {
    const { 
      enabled, 
      patterns, 
    ...
    const blacklistData = {
      ...
      patterns: Array.isArray(patterns) ? patterns.filter(p => p && typeof p === 'string') : [],
      whitelistWords: Array.isArray(whitelistWords) ? whitelistWords.filter(w => w && typeof w === 'string') : [],
      whitelistChannels: Array.isArray(whitelistChannels) ? whitelistChannels.filter(c => c && typeof c === 'string') : [],
      whitelistCategories: Array.isArray(whitelistCategories) ? whitelistCategories.filter(c => c && typeof c === 'string') : [],
      bypassRoles: Array.isArray(bypassRoles) ? bypassRoles.filter(r => r && typeof r === 'string') : [],
      logsChannelId: logsChannelId || '',
      notificationMessage: notificationMessage || '{user}, you used a blacklisted word!'
    };

    const settings = await Settings.findOneAndUpdate(
      { guildId: req.user.guildId },
      { 
        $set: { blacklistWords: blacklistData },
        $setOnInsert: { guildId: req.user.guildId }
      },
      { new: true, upsert: true }
    );

    if (global.client) {
      global.client.emit('blacklistWordsUpdated', req.user.guildId, settings.blacklistWords);
    }

    res.json({ success: true, settings: settings.blacklistWords });
  } catch (error) {
    console.error('[ERROR] Failed to save blacklist words settings:', error);
    res.status(500).json({ error: 'Failed to save blacklist words settings' });
  }
});
