...
const hasSupportRoleForType = async (userId, ticketType, guildId) => {
    try {
...
const buildBaseQuery = (userId, userRoles, config, guildId) => {
    const query = { guildId };
    
    let isSupport = false;
    if (config.TicketTypes) {
        for (const [_, ticketType] of Object.entries(config.TicketTypes)) {
            if (ticketType.SupportRole) {
                let supportRoles = ticketType.SupportRole;
                if (!Array.isArray(supportRoles)) {
                    supportRoles = [supportRoles];
                }
                const matches = supportRoles.filter(roleId => userRoles.includes(roleId));
                if (matches.length > 0) {
                    isSupport = true;
                    break;
                }
...
const extractRatingValue = (ticket) => {
    if (ticket.reviewData && typeof ticket.reviewData === 'object') {
        for (const [key, val] of Object.entries(ticket.reviewData)) {
            if (key === 'additionalFeedback') continue;
...
const hasValidRating = (ticket) => {
    return extractRatingValue(ticket) !== null;
};
...
const calculateSatisfactionRate = async (baseQuery) => {
    const ratedTickets = await Ticket.find({
        ...
        $or: [
            { rating: { $regex: /⭐.*\(\d\/5\)$/ } },
            { reviewData: { $exists: true, $ne: null } }
        ]
    });

    const validRatedTickets = ratedTickets.filter(ticket => hasValidRating(ticket));

    if (validRatedTickets.length === 0) return 0;

    const goodRatings = validRatedTickets.filter(ticket => {
        const rating = extractRatingValue(ticket);
        return rating !== null && rating >= 4;
    }).length;

    return Math.round((goodRatings / validRatedTickets.length) * 100 * 100) / 100;
};
...
router.get('/list', auth, async(req, res) => {
    try {
        ...
        const baseQuery = buildBaseQuery(userId, userRoles, config, guildId);
                        ...
                        avgResponseTime: {
                            $avg: {
                                $cond: [
                                    ...
                                    {
                                        $subtract: [
                                            { $arrayElemAt: [{ $ifNull: ['$messages.timestamp', []] }, 1] },
                                            '$createdAt'
                                        ]
                                    },
                                    null
...
router.get('/dashboard', auth, cacheStats, async (req, res) => {
    try {
        ...
        const validRatedTickets = stats.satisfactionStats.filter(ticket => hasValidRating(ticket));
        const goodRatings = validRatedTickets.filter(ticket => {
            const rating = extractRatingValue(ticket);
            return rating !== null && rating >= 4;
        }).length;
        const satisfactionRate = validRatedTickets.length > 0
            ? Math.round((goodRatings / validRatedTickets.length) * 100 * 100) / 100
            : 0;
...
router.get('/:id/transcript', auth, async(req, res) => {
    try {
        ...
        const userId = req.user.id;
        const guildId = req.user.guildId;
        const hasSupport = await hasSupportRoleForType(userId, ticket.ticketType, guildId);
        ...
        if (ticket.reviewData && typeof ticket.reviewData === 'object') {
            const entries = Object.entries(ticket.reviewData).filter(([key]) => key !== 'additionalFeedback');
            if (entries.length > 0) {
                const [firstKey, firstVal] = entries[0];
                if (firstVal && typeof firstVal === 'object' && firstVal.value) {
                    ratingDisplay = `${firstVal.emoji || ''} ${firstVal.label || ''} (${firstVal.value}/5)`.trim();
                }
            }
            feedbackDisplay = ticket.reviewData.additionalFeedback || '';
            reviewData = ticket.reviewData;
        } else if (ticket.rating && ticket.rating !== 'No Rating Yet') {
            ratingDisplay = ticket.rating;
            feedbackDisplay = ticket.reviewFeedback || '';
        }
...
router.post('/:ticketId/send-message', auth, async (req, res) => {
    try {
        ...
        const hasPermission = await hasSupportRoleForType(userId, ticket.ticketType, guildId);
        if (!hasPermission) {
...
router.post('/claim/:ticketId', auth, async (req, res) => {
    try {
        ...
        const hasSupportRole = supportRoles.some(roleId => userRoles.includes(roleId));
        if (!hasSupportRole) {
        ...
        if (ticket.claimed) {
            if (ticket.claimedBy === userId) {
                ...
                if (ticketMessage && ticketMessage.components?.length > 0 && ticketMessage.components[0]?.components?.length > 0) {
            ...
            if (ticketMessage && ticketMessage.components?.length > 0 && ticketMessage.components[0]?.components?.length > 0) {
                ...
                const components = ticketMessage.components[0].components.map(c => {
                    if (c.data.custom_id?.startsWith('ticketclaim')) {
                        return new ButtonBuilder()
                            .setCustomId(c.data.custom_id)
                            .setLabel((lang?.Tickets?.Claim?.ClaimedByButton || 'Claimed by {claimer}').replace('{claimer}', member.user.username))
                            .setStyle(ButtonStyle[ticketType.Claiming.Button.Style] || ButtonStyle.Secondary)
                            .setEmoji(ticketType.Claiming.Button.Emoji || '🎫');
                    }
                    return ButtonBuilder.from(c.data);
        ...
        return res.json({ 
            ...
            ticket: {
                ...
                assignee: updatedTicket.claimedBy
            }
        });
    } catch (error) {
...
router.post('/close/:ticketId', auth, async (req, res) => {
    try {
        ...
        const { handleTicketClose } = require('../../../../utils/tickets/ticketClosure.js');
