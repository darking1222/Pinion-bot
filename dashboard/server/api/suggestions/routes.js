...
router.get('/', auth, checkSuggestionPermissions, async (req, res) => {
    try {
        ...
        const cacheKey = `suggestions_${guildId}`;
        ...
        const query = guildId ? { $or: [{ guildId }, { guildId: { $exists: false } }, { guildId: null }] } : {};
        const suggestions = await Suggestion.find(query)
            .sort({ createdAt: -1 })
            .lean();

        cache.set(cacheKey, suggestions, cache.TTL.SUGGESTIONS);
...
router.post('/:id/accept', auth, checkSuggestionPermissions, async (req, res) => {
    try {
        ...
        let channel;
        try {
            channel = await client.channels.fetch(suggestion.channelId);
        } catch (err) {
            return res.status(404).json({ error: 'Channel not found or inaccessible' });
        }
        ...
        await suggestionActions.acceptSuggestion(client, mockInteraction, id, reason || 'No reason provided');
        
        const guildId = req.user?.guildId;
        cache.store.delete(`suggestions_${guildId}`);

        const updatedSuggestion = await Suggestion.findOne({ uniqueId: id });
        res.json(updatedSuggestion || suggestion);
    } catch (error) {
...
router.post('/:id/deny', auth, checkSuggestionPermissions, async (req, res) => {
    try {
        ...
        await suggestionActions.denySuggestion(client, mockInteraction, id, reason || 'No reason provided');
        ...
        res.status(500).json({ error: error.message || 'Failed to deny suggestion' });
    }
});
